...
class WorkflowActionRuleTask(models.Model):
    ...
    create_model = fields.Selection(selection_add=[('purchase.order', "Purchase Order")])

    def create_record(self, documents=None):
        rv = super(WorkflowActionRuleTask, self).create_record(documents=documents)
        if self.create_model == 'purchase.order':
            prapare_values = {
                'name': "new PO from Documents",
                'partner_id': self.env.user.partner_id.id,
                'date_order': fields.Datetime.now(),
            }
            new_obj = self.env[self.create_model].create(prapare_values)
            sale_action = {
